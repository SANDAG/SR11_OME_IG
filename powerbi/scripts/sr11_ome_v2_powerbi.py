...
for index, row in runs.iterrows():
    print(row['folder'])
    filename = "T:\\projects\\sr12\\OWP\\sr11ome\\runs\\New_Runs\\" + row['folder'] + '\\data_out\\' + 'poe_results.csv'
    print(filename)
...
#################################################################################
# sim volume
table_volume = pd.pivot_table(data, values='sim_volume', index=['hour','port_name', 'direction','vehicle_type','run'],
...
#################################################################################
# sim wait times
table_wait = pd.pivot_table(data, values='sim_wait_time', index=['hour', 'port_name', 'direction', 'vehicle_type','run'],
                            columns=['traveler_type'], aggfunc=np.sum)
...
#################################################################################
# sim open lanes
table_open_lanes = pd.pivot_table(data, values='sim_open_lanes', index=['hour', 'port_name', 'direction', 'vehicle_type','run'],
                            columns=['traveler_type'], aggfunc=np.sum)
...
#################################################################################
# sim toll
table_toll = pd.pivot_table(data, values='toll', index=['hour', 'port_name', 'direction', 'vehicle_type','run'],
                            columns=['traveler_type'], aggfunc=np.sum)
...
#################################################################################
result = pd.merge(table_volume, table_wait, on=['hour', 'port_name', 'direction', 'vehicle_type','run'])
result = pd.merge(result, table_open_lanes, on=['hour', 'port_name', 'direction', 'vehicle_type','run'])
result = pd.merge(result, table_toll, on=['hour', 'port_name', 'direction', 'vehicle_type','run'])
